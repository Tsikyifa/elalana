...
type Props = {
  message: MessageMarcheItem
  onEdit: (m: MessageMarcheItem) => void
  onDelete: (m: MessageMarcheItem) => void
}
...
export default function MessageRowActions({ message, onEdit, onDelete }: Props) {
  ...
  return (
    <>
      <button
        ref={triggerRef}
        type="button"
        className={`icon-button${position ? ' is-open' : ''} message-actions-trigger`}
        aria-haspopup="menu"
        ...
        <>
          ...
          >
            <button
              ...
              onMouseDown={(e) => e.stopPropagation()}
              onClick={(e) => { e.stopPropagation(); e.preventDefault(); run(() => onEdit(message)) }}
            >
            ...
            <button
              ...
              onMouseDown={(e) => e.stopPropagation()}
              onClick={(e) => { e.stopPropagation(); e.preventDefault(); run(() => onDelete(message)) }}
            >
